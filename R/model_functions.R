...
generate_Y_model <- function(X, distr = DISTR,
                             df, sigma_fun, df_fun) {
    ...
    switch(distr,
        ...
        "pareto" = {
            Y_tilde <- rpareto(n, alpha = df_fun(X, df))
        },
        "frechet" = {
            Y_tilde <- rfrechet(n, alpha = df_fun(X, df))
        }
    )
...
generate_quantile_model <- function(quantiles, X,
    ...
    switch(distr,
        ...
        "pareto" = {
            q_tilde <- quantiles_pareto(quantiles, df_fun(X, df))
        },
        "frechet" = {
            q_tilde <- quantiles_frechet(quantiles, df_fun(X, df))
        }
    )
...
quantiles_frechet <- function(quantiles, alpha) {
    ## numeric_vector numeric_vector -> numeric_matrix
    ## produce matrix with Frechet quantiles where nrow = length(alpha) and
    ## ncol = length(quantiles)
    ## Note: alpha = 1 / shape from GPD

    purrr::map(quantiles, function(q) {
        evd::qfrechet(q, shape = alpha)
    }) %>%
        list2matrix(dim = "col")
}
...
rfrechet <- function(n, alpha) {
    ...
    ## sample `n` observations from Frechet distribution with `alpha` parameter
    ## Note: alpha = 1 / shape from GPD

    if (length(alpha) > 1 & length(alpha) != n) {
        stop(glue::glue("The alpha parameter must be of length {n} or 1."))
    }

    evd::rfrechet(n, shape = alpha)
}
quantiles_pareto <- function(quantiles, alpha) {
    ## numeric_vector numeric_vector -> numeric_matrix
    ## produce matrix with Pareto quantiles where nrow = length(alpha) and
    ## ncol = length(quantiles)
    ## Note: alpha = 1 / shape from GPD
...
rpareto <- function(n, alpha) {
    ## numeric numeric_vector -> numeric_vector
    ## sample `n` observations from Pareto distribution with `alpha` parameter
    ## Note: alpha = 1 / shape from GPD
    if (length(alpha) > 1 & length(alpha) != n) {
        stop(glue::glue("The alpha parameter must be of length {n} or 1."))
    }
...
generate_joint_distribution_internal <- function(n, p,
    ...
    switch(model,
        ...
        "iid_step" = {
            Y <- generate_Y_model(X, distr, df, sigma_iid, df_step)
        },
        "step_step" = {
