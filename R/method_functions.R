fit_shape_rf <- function(X, Y, Q_X,
    ...
    ##  -> shape_rf
    ## fits the Hill's using a regression forest on the log exceedances
    ##  that we name Hill's exponential

    min.node.size <- fit_erf$min.node.size

    exc_ind <- which(Y > Q_X & Q_X > 0)
    X_ <- X[exc_ind, ]
    Y_ <- log(Y[exc_ind] / Q_X[exc_ind])

    # qqplot(rexp(length(Y_)), Y_)
    # abline(a = 0, b = 1)

    fitted_shape_rf <- grf::regression_forest(X_,
        Y_,
...
predict_shape_rf <- function(
    object, newdata = NULL,
    quantiles = c(0.95, 0.99)) {
    ## hill numeric_matrix numeric_vector -> numeric_matrix
    ## predicts extreme quantiles using Hill's exponential estimator
    ...
    # predict shape parameter xi(x)
    xi_shape_rf <- predict(
        object = object$fit_shape_rf, newdata = newdata
    )[, 1]


    hill_pars <- tibble::tibble(
        "sigma" = xi_shape_rf * Q_x,
        "xi" = xi_shape_rf
    )
    ...
    # predict quantiles
    list(
        preds = erf:::predict_extreme_quantiles(
            hill_pars,
            Q_x,
            quantiles,
...
fit_hill <- function(X, Y, Q_X,
    ...
    ##  numeric numeric numeric numeric numeric_vector boolean
    ##  -> hill
    ## fits the weighted Hill's estimator
...
predict_hill <- function(object, newdata = NULL, quantiles = c(0.95, 0.99)) {
    ## hill numeric_matrix numeric_vector -> numeric_matrix
    ## predicts extreme quantiles using weighted Hill's estimator
    ...
    # predict Hill's shape parameter xi(x)
    Y <- object$fit_erf$quantile_forest$Y.orig
    Q_X <- object$fit_erf$Q_X
    W <- as.matrix(grf::get_forest_weights(
        forest = object$fit_erf$quantile_forest,
        newdata = newdata
    ))
    ...
    xi_hill <- tibble::tibble(xi = numeric(0)) %>%
        dplyr::bind_rows(purrr::map_dfr(seq_len(ntest), function(i) {
            wi_x <- W[i, ]
            wi_x_norm <- wi_x / sum(wi_x)
            # option 1: paper
            tibble::tibble(xi = sum(wi_x_norm * log(1 + Z / Q_x[i])))
            # option 2
            # tibble(xi = sum(wi_x_norm * log(1 + Z/Q_X[exc_ind])))
        }))
    # ... until here

    hill_pars <- tibble::tibble(
        "sigma" = xi_hill$xi * Q_x,
        "xi" = xi_hill$xi
    )
...
# !!! to update
hill_param <- function(object, newdata = NULL) {
    ## hill numeric_matrix numeric_vector -> tibble
    ## predicts hill shape parameter
    ...
    xi_hill <- tibble::tibble(xi = numeric(0)) %>%
        dplyr::bind_rows(purrr::map_dfr(seq_len(ntest), function(i) {
            wi_x <- W[i, ]
            wi_x_norm <- wi_x / sum(wi_x)
            tibble::tibble(xi = sum(wi_x_norm * log(1 + Z / Q_x[i]))) # option 1: paper
            # tibble(xi = sum(wi_x_norm * log(1 + Z/Q_X[exc_ind]))) #option 2
        }))
...
fit_taillardat <- function(X, Y,
    ...
    ##  -> taillardat_qrf
    ## fit a QRF with the method proposed by Taillardat et al. (2018)
    ## Copyright: Taillardat et al. (2018)
...
fit_predict_wrapper <- function(dat_train, dat_test, response,
    ...
    pred_erf <- predict(
        object = fit_erf, newdata = mat_test$X,
        quantiles = quantiles
    )
