...
gg4 <- ggplot(dat) +
    geom_point(aes(x = X1, y = Y), color = my_palette$grey, alpha = 0.5) +
    annotate("text",
        x = x_text, y = max(dat$q2),
        label = TeX("$Q_x(0.8)$"),
        color = my_palette$background,
        size = text_size
    ) +
    annotate("text",
        x = x_text, y = max(dat$q5),
        label = TeX("$Q_x(0.995)$"),
        color = my_palette$background,
        size = text_size
    ) +
...
gg9 <- gg8 +
    ...
    annotate(
        geom = "text", x = 2, y = .31,
        label = TeX("$u = Q_x(\\tau_n)$"), hjust = "left"
    )
...
fitted_quantiles <- tibble(
    ...
    mutate(
        exceed = if_else(Y > Q_interm, TRUE, FALSE),
        Z = if_else(exceed, Y, NaN)
    )
...
gg13_bis <- gg13 +
  geom_vline(aes(xintercept = x_test), linewidth = 1, linetype = 2,
...
gg14 <- gg13 +
  geom_vline(aes(xintercept = x_test), linewidth = 1, linetype = 2,
             color = my_palette2$red) +
    geom_point(aes(x = x_test, y = qq_test[, 2]), shape = 19, size = 2)
